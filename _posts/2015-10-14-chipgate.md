...
<p>
  <figure>
    ...
    <figcaption>The image shows violin plots for the location of the Gaussians (in Geekbench's proprietory battery score units).</figcaption>
  </figure>
</p>

The statistical model I defined expresses my belief that the battery score observations are distributed according to a mixture of two distinct components, one for each iPhone chip. In particular, I assume that the observations of each chip follow one of two distinct Gaussian distributions, where the location and the variance of these Gaussians are believed to differ with the chip type. The plot shows the statistical distributions of the locations of the Gaussians that I inferred from the Geekbench data. The depicted violin shapes are kernel density plots. They reflect the degree of belief in a particular value of the locations. The violins illustrate that, given the observed data, there is remaining uncertainty with respect to these locations. However, despite this uncertainty, the locations of the two chip classes are well separated in value, and the TSMC chip clearly has the better performance (Samsung chips are marked as N71AP in the iPhone 6S, whereas TSMC models are named N71mAP).
...
However, I am unable to say if this performance discrepancy is relevant for daily usage. [Apple claims it's not](http://www.macrumors.com/2015/10/08/apple-a9-chip-2-3-percent-difference/). So do [several](http://arstechnica.com/apple/2015/10/samsung-vs-tsmc-comparing-the-battery-life-of-two-apple-a9s/) [news](http://9to5mac.com/2015/10/12/a9-battery-test/) [websites](http://www.heise.de/mac-and-i/meldung/Akkulaufzeiten-beim-iPhone-6s-Apple-hat-recht-2842072.html) that tested the iPhone 6S under "real world" conditions. However, as with the battery performance claims, these new studies suffer from small sample bias, and should be taken with a grain of salt.
